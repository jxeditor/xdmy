...
public class IncomingDao extends BaseDao implements IIncomingDao {
    ...
    @Override
    public List<Incoming> findAllIncoming(int pageNum, int pageSize, String producerName, String productName,
            String bizStartDate, String bizEndDate, String companyName) {
        int currOffset = (pageNum - 1) * pageSize;
        String sql = "SELECT * FROM incoming WHERE 1=1 AND is_delete = 0 AND company_name = ?";
        sql = genFilterSql(sql, producerName, productName, bizStartDate, bizEndDate);
        sql += " ORDER BY billdate DESC,odd DESC,id DESC LIMIT ? ,?";
        return jdbcTemplate.query(sql, new IncomingRowMapper(), companyName, currOffset, pageSize);
    }

    @Override
    public List<Incoming> getIncomingStatement(String producerName, String bizStartDate, String bizEndDate,
            String companyName) {
        String sql = "SELECT * FROM incoming WHERE 1=1 AND is_delete = 0 AND paystatus != 2 AND company_name = ?" +
                " AND producer = ? AND billdate >= ? AND billdate <= ?";
        sql += " ORDER BY billdate,odd,id";
        return jdbcTemplate.query(sql, new IncomingRowMapper(), companyName, producerName, bizStartDate, bizEndDate);
    }

    @Override
    public int getDistinctSize(String producerName, String bizStartDate, String bizEndDate, String companyName) {
        String sql = "SELECT count(distinct odd) FROM incoming WHERE 1=1 AND is_delete = 0 AND paystatus != 2 AND company_name = ?"
                +
                " AND producer = ? AND billdate >= ? AND billdate <= ?";
        return jdbcTemplate.queryForObject(sql, Integer.class, companyName, producerName, bizStartDate, bizEndDate);
    }

    @Override
    public double getSumPay(String producerName, String bizStartDate, String bizEndDate, String companyName) {
        String sql = "SELECT sum(money) FROM incoming WHERE 1=1 AND is_delete = 0 AND paystatus != 2 AND company_name = ?"
                +
                " AND producer = ? AND billdate >= ? AND billdate <= ?";
        return jdbcTemplate.queryForObject(sql, Double.class, companyName, producerName, bizStartDate, bizEndDate);
    }

    @Override
    public int getAllTotalSize(String producerName, String productName, String bizStartDate, String bizEndDate,
            String companyName) {
        String sql = "SELECT count(1) FROM incoming WHERE 1=1 AND is_delete = 0 AND company_name = ?";
        sql = genFilterSql(sql, producerName, productName, bizStartDate, bizEndDate);
    ...
    public int addIncoming(Incoming incoming, String materialRelationsStr, String companyName) {
        ...
        try {
            // 添加入货记录
            String sql = "INSERT INTO incoming(odd,producer,product,billdate,amount,unitprice,money,paystatus,remark,is_delete,operate_material,company_name) "
                    +
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            jdbcTemplate.update(con -> {
                ...
                ps.setString(12, incoming.getCompany_name() != null ? incoming.getCompany_name() : "");
                return ps;
            }, keyHolder);
            ...
            Integer productCount = jdbcTemplate.queryForObject(checkProductSql, Integer.class, incoming.getProduct(), companyName);
            ...
            if (productCount == 0) {
                ...
                String insertProductSql = "INSERT INTO product(product_name, suggested_price, cost_price, maintain_material, company_name) "
                        +
                        "VALUES(?, ?, ?, 0, ?)";
                jdbcTemplate.update(insertProductSql, incoming.getProduct(), incoming.getUnitprice(),
                        incoming.getUnitprice(), companyName);
            }

            // 检查stock表中是否存在对应产品的记录
            String checkSql = "SELECT COUNT(*) FROM stock WHERE product = ? AND company_name = ?";
            Integer count = jdbcTemplate.queryForObject(checkSql, Integer.class, incoming.getProduct(), companyName);
            count = count != null ? count : 0;

            if (count == 0) {
                // 如果不存在，插入一条新记录
                String insertSql = "INSERT INTO stock(product, unitstock, unitprice, purchaseprice, inamount, outamount, lastindate, stockstatus, company_name) "
                        +
                        "VALUES(?, 0, ?, ?, ?, 0, ?, '1', ?)";
                jdbcTemplate.update(insertSql, incoming.getProduct(), incoming.getUnitprice(), incoming.getUnitprice(),
                        incoming.getAmount(), incoming.getBilldate(), companyName);
            } else {
                // 如果存在，更新inamount、purchaseprice和lastindate字段
                String updateSql = "UPDATE stock SET inamount = inamount + ?, purchaseprice = ?, lastindate = ? WHERE product = ? AND company_name = ?";
                jdbcTemplate.update(updateSql, incoming.getAmount(), incoming.getUnitprice(), incoming.getBilldate(),
                        incoming.getProduct(), companyName);
            }
            ...
            if (incoming.getOperate_material() == 1) {
                ...
                if (materialRelationsStr != null && !materialRelationsStr.isEmpty()) {
                    ...
                    try {
                        com.alibaba.fastjson.JSONArray jsonArray = com.alibaba.fastjson.JSON
                                .parseArray(materialRelationsStr);
                ...
                if (relations.isEmpty()) {
                    ...
                    relations = jdbcTemplate.queryForList(relationSql, incoming.getProduct(), companyName);
                }
                ...
                for (java.util.Map<String, Object> relation : relations) {
                    ...
                    // 记录原材料操作
                    String operationSql = "INSERT INTO incoming_material_operation(incoming_id, material_name, quantity, operation_date, company_name) VALUES(?, ?, ?, ?, ?)";
    ...
    @Override
    public int addIncoming(Incoming incoming, String companyName) {
        return addIncoming(incoming, null, companyName);
    }
    ...
    public int deleteIncomingById(int id, String companyName) {
        ...
        try {
            // 先查询要删除的入货记录的产品和数量
            String querySql = "SELECT * FROM incoming WHERE id = ? AND company_name = ?";
            Incoming incoming = jdbcTemplate.queryForObject(querySql, new IncomingRowMapper(), id, companyName);
            ...
            // 删除入货记录
            String sql = "UPDATE incoming set is_delete = 1 WHERE id = ? AND company_name = ?";
            ...
            String updateSql = "UPDATE stock SET inamount = inamount - ?, " +
                    "purchaseprice = (SELECT unitprice FROM incoming WHERE product = ? AND is_delete = 0 AND company_name = ? ORDER BY billdate DESC LIMIT 1), "
                    +
                    "lastindate = (SELECT MAX(billdate) FROM incoming WHERE product = ? AND is_delete = 0 AND company_name = ?) "
                    +
                    "WHERE product = ?";
            jdbcTemplate.update(updateSql, incoming.getAmount(), incoming.getProduct(), companyName,
                    incoming.getProduct(), companyName, incoming.getProduct());
    ...
    public int updateIncoming(Incoming incoming, String materialRelationsStr, String companyName) {
        ...
        try {
            ...
            int result = jdbcTemplate.update(sql, incoming.getOdd(), incoming.getProducer(), incoming.getProduct(),
                    incoming.getBilldate(), incoming.getAmount(), incoming.getUnitprice(), incoming.getMoney(),
                    incoming.getPaystatus(), incoming.getRemark(), incoming.getOperate_material(), incoming.getId(),
                    companyName);
            ...
            if (oldIncoming.getProduct().equals(incoming.getProduct())) {
                ...
                jdbcTemplate.update(updateSql, amountDiff, incoming.getUnitprice(), incoming.getBilldate(),
                        incoming.getProduct());
            } else {
                ...
                String updateOldSql = "UPDATE stock SET inamount = inamount - ?, " +
                        ...
                        +
                        "WHERE product = ?";
                jdbcTemplate.update(updateOldSql, oldIncoming.getAmount(), oldIncoming.getProduct(), companyName,
                        oldIncoming.getProduct(), companyName, oldIncoming.getProduct());

                // 检查新产品是否存在于product表中
                String checkProductSql = "SELECT COUNT(*) FROM product WHERE product_name = ?";
                Integer productCount = jdbcTemplate.queryForObject(checkProductSql, Integer.class,
                        incoming.getProduct());
                productCount = productCount != null ? productCount : 0;
                ...
                if (count == 0) {
                    ...
                    String insertSql = "INSERT INTO stock(product, unitstock, unitprice, purchaseprice, inamount, outamount, lastindate, stockstatus) "
                            +
                            "VALUES(?, 0, ?, ?, ?, 0, ?, '1')";
                    jdbcTemplate.update(insertSql, incoming.getProduct(), incoming.getUnitprice(),
                            incoming.getUnitprice(), incoming.getAmount(), incoming.getBilldate());
                } else {
                    // 如果存在，更新inamount、purchaseprice和lastindate字段
                    String updateNewSql = "UPDATE stock SET inamount = inamount + ?, purchaseprice = ?, lastindate = ? WHERE product = ?";
                    jdbcTemplate.update(updateNewSql, incoming.getAmount(), incoming.getUnitprice(),
                            incoming.getBilldate(), incoming.getProduct());
                }
            }
            ...
            if (incoming.getOperate_material() == 1) {
                ...
                if (relations.isEmpty()) {
                    String relationSql = "SELECT material_name, quantity FROM product_material_relation WHERE product_name = ? AND is_default = 1 AND company_name = ?";
                    relations = jdbcTemplate.queryForList(relationSql, new Object[] { incoming.getProduct(), companyName });
                }
                ...
                java.util.List<java.util.Map<String, Object>> oldOperations = jdbcTemplate.queryForList(oldOperationSql,
                        incoming.getId());
                ...
                for (java.util.Map<String, Object> relation : relations) {
                    ...
                    if (oldMaterialQuantities.containsKey(materialName)) {
                        ...
                        String restoreSql = "UPDATE material_stock SET unitstock = unitstock + ? WHERE material_name = ? AND company_name = ?";
                        jdbcTemplate.update(restoreSql, oldQuantity, materialName, companyName);
                    }
                    ...
                    // 记录原材料操作
                    String operationSql = "INSERT INTO incoming_material_operation(incoming_id, material_name, quantity, operation_date, company_name) VALUES(?, ?, ?, ?, ?)";
                    jdbcTemplate.update(operationSql, incoming.getId(), materialName, totalQuantity,
                            new java.util.Date(), companyName);
                }
    ...
    public int updateIncoming(Incoming incoming) {
        return updateIncoming(incoming);
    }
    ...
    public String genFilterSql(String sql, String producerName, String productName, String bizStartDate,
            String bizEndDate) {
    ...
    @Override
    public List<String> findProducerNamesByPrefix(String prefix, int pageNum, int pageSize, String companyName) {
        int offset = (pageNum - 1) * pageSize;
        // 优先匹配包含完整前缀的供应商，然后匹配包含前缀中每个字符的供应商
        String sql = "SELECT DISTINCT producer FROM incoming " +
                "WHERE is_delete = 0 AND company_name = ? AND (producer LIKE ? OR producer LIKE ?) " +
                "ORDER BY " +
                "CASE " +
    ...
    @Override
    public int getProducerNamesCount(String prefix, String companyName) {
    ...
    @Override
    public List<Map<String, Object>> getIncomingMaterialOperations(int id) {
        // 先查询入货记录是否属于该公司
        String checkSql = "SELECT COUNT(*) FROM incoming WHERE id = ?";
